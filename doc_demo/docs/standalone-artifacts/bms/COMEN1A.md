# BMS English Documentation: COMEN1A

## 1. Screen Purpose

`COMEN1A` is a BMS screen map in mapset `COMEN01` with map name `COMEN1A`. It is associated with program `COMEN01C` and contains 28 extracted field(s) across rows 1 through 24.
...
- Layout labels sampled from the map: Tran:, Date:, Prog:, Time:, Main Menu, Please select an option :, ENTER=Continue  F3=Exit
- Color usage: None found in extracted data

## 6. Program and Navigation Context

The extracted program link is `COMEN01C`. In a CICS migration, this screen should be analyzed with the program's SEND MAP and RECEIVE MAP commands to determine submit, validation, and navigation behavior.
