# BMS English Documentation: COUSR2A

## 1. Screen Purpose

`COUSR2A` is a BMS screen map in mapset `COUSR02` with map name `COUSR2A`. It is associated with program `COUSR02C` and contains 28 extracted field(s) across rows 1 through 24.
...
- `FNAME` at row 11, column 18, length 20: terminal input field with attributes `(FSET,NORM,UNPROT)`.
- `LNAME` at row 11, column 56, length 20: terminal input field with attributes `(FSET,NORM,UNPROT)`.
- `PASSWD` at row 13, column 16, length 8: terminal input field with attributes `(DRK,FSET,UNPROT)`.
- `USRTYPE` at row 15, column 17, length 1: terminal input field with attributes `(FSET,NORM,UNPROT)`.
...
- Layout labels sampled from the map: Tran:, Date:, Prog:, Time:, Update User, Enter User ID:, *********************************************** DFHMDF ATTRB=(ASKIP,NORM), COLOR=TURQUOISE, LENGTH=11, POS=(11,6), INITIAL=, Last Name:, Password:, (8 Char), User Type:, (A=Admin, U=User), ENTER=Fetch  F3=Save&&Exit  F4=Clear  F5=Save F12=Cancel
- Color usage: None found in extracted data

## 6. Program and Navigation Context

The extracted program link is `COUSR02C`. In a CICS migration, this screen should be analyzed with the program's SEND MAP and RECEIVE MAP commands to determine submit, validation, and navigation behavior.
