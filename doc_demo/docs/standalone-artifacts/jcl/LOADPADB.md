# JCL English Documentation: LOADPADB

## 1. Executive Summary

`LOADPADB` is a standalone batch job defined in `LOADPADB.JCL`. The extracted job description is: M2APP. It contains 1 execution step(s), reading 5 input dataset reference(s) and writing 0 output dataset reference(s).

## 2. Batch Runtime Context

- Source file: `carddemo\app\app-authorization-ims-db2-mq\jcl\LOADPADB.JCL`
- Job class: `A`
- Message class: `H`
...
This step executes `DFSRRC00` and runs an IMS dependent region program. It has 11 DD statement(s).
Extracted step notes: YLIBS JCLLIB ORDER=VIPINGP.CNTL.PROCLIB EXECUTE IMS PROGRAM
- Inputs: OEMA.IMS.IMSP.SDFSRESL, OEM.IMS.IMSP.PSBLIB, AWS.M2.CARDDEMO.PAUTDB.ROOT.FILEO, AWS.M2.CARDDEMO.PAUTDB.CHILD.FILEO, OEMPP.IMS.V15R01MB.PROCLIB(DFSVSMDB)
- Outputs: None found in extracted data

## 4. Dataset and Dependency Context

- Input datasets: OEMA.IMS.IMSP.SDFSRESL, OEM.IMS.IMSP.PSBLIB, AWS.M2.CARDDEMO.PAUTDB.ROOT.FILEO, AWS.M2.CARDDEMO.PAUTDB.CHILD.FILEO, OEMPP.IMS.V15R01MB.PROCLIB(DFSVSMDB)
...
- Peer/upstream references to the flat-file inputs: DBPAUTP0/UNLOAD, DBPAUTP0/UNLOAD, UNLDGSAM/STEP01, UNLDGSAM/STEP01, UNLDPADB/STEP01, UNLDPADB/STEP01, UNLDPADB/STEP0, UNLDPADB/STEP01
