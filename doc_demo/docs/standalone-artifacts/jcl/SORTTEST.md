# JCL English Documentation: SORTTEST

## 1. Executive Summary

`SORTTEST` is a standalone batch job defined in `SORTTEST.jcl`. The extracted job description is: SORT STEP TESTING. It contains 1 execution step(s), reading 1 input dataset reference(s) and writing 1 output dataset reference(s).

## 2. Batch Runtime Context

- Source file: `carddemo\samples\jcl\SORTTEST.jcl`
- Job class: `A`
- Message class: `0`
...
- Outputs: AWS.M2.CARDDEMO.TRANSACT.DALY(+1)
- Inline SYSIN: 6 control line(s) are supplied to the step.

## 4. Dataset and Dependency Context

- Input datasets: AWS.M2.CARDDEMO.TRANSACT.BKUP(0)
- Output datasets: AWS.M2.CARDDEMO.TRANSACT.DALY(+1)
...
- Peer/upstream references to the flat-file inputs: COMBTRAN/STEP05R, TRANREPT/STEP05R, TRANREPT/STEP10R
