# JCL English Documentation: CBLDBMS

## 1. Executive Summary

`CBLDBMS` is a standalone batch job defined in `BMSCMP.jcl`. The extracted job description is: Compile BMS Map. It contains 2 execution step(s), reading 1 input dataset reference(s) and writing 2 output dataset reference(s).

## 2. Batch Runtime Context

- Source file: `carddemo\samples\jcl\BMSCMP.jcl`
- Job class: `A`
- Message class: `H`
- Application programs/procedures: BUILDBMS, SDSF
...
This step executes `BUILDBMS` and expands and runs a cataloged procedure. It has 0 DD statement(s).
...
Extracted step notes: CICS commands in batch to Execute NEWCOPY
