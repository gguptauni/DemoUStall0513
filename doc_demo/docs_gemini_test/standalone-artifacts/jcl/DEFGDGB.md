# JCL English Documentation: DEFGDGB

## 1. Executive Summary

`DEFGDGB` is a standalone batch job defined in `DEFGDGB.jcl`. The extracted job description is: DEF GDG BASES. It contains 1 execution step(s), reading 1 input dataset reference(s) and writing 0 output dataset reference(s).

## 2. Batch Runtime Context

- Source file: `carddemo\app\jcl\DEFGDGB.jcl`
- Job class: `A`
- Message class: `0`
...
Extracted step notes: DEFINE GDG BASES NEEDED BY CARDDEMO PROJECT
- Inputs: inline control data
- Outputs: None found in extracted data
- Inline SYSIN: 36 control line(s) are supplied to the step.
