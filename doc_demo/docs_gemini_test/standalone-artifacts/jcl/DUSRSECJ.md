# JCL English Documentation: DUSRSECJ

## 1. Executive Summary

`DUSRSECJ` is a standalone batch job defined in `DUSRSECJ.jcl`. The extracted job description is: DEF USRSEC FILE. It contains 4 execution step(s), reading 4 input dataset reference(s) and writing 1 output dataset reference(s).

## 2. Batch Runtime Context

- Source file: `carddemo\app\jcl\DUSRSECJ.jcl`
- Job class: `A`
- Message class: `H`
...
- Outputs: AWS.M2.CARDDEMO.USRSEC.PS
...
Extracted step notes: ------------------------------------------------------------------- DEFINE VSAM FILE FOR USER SECURITY
...
Extracted step notes: ------------------------------------------------------------------- COPY USER SECURITY DATA FROM PS TO VSAM FILE
- Inputs: AWS.M2.CARDDEMO.USRSEC.PS, AWS.M2.CARDDEMO.USRSEC.VSAM.KSDS, inline control data
- Outputs: None found in extracted data
- Inline SYSIN: 1 control line(s) are supplied to the step.

## 4. Dataset and Dependency Context

- Input datasets: SYSIN, inline control data, AWS.M2.CARDDEMO.USRSEC.PS, AWS.M2.CARDDEMO.USRSEC.VSAM.KSDS
- Output datasets: AWS.M2.CARDDEMO.USRSEC.PS
