# JCL English Documentation: TRANTYPE

## 1. Executive Summary

`TRANTYPE` is a standalone batch job defined in `TRANTYPE.jcl`. The extracted job description is: DEFINE TRAN TYPE. It contains 3 execution step(s), reading 3 input dataset reference(s) and writing 0 output dataset reference(s).

## 2. Batch Runtime Context

- Source file: `carddemo\app\jcl\TRANTYPE.jcl`
- Job class: `A`
- Message class: `0`
...
Extracted step notes: DELETE TRANSACATION TYPE VSAM FILE IF ONE ALREADY EXISTS
...
This step executes `IDCAMS` and manages VSAM/catalog resources. It has 2 DD statement(s).
Extracted step notes: DEFINE TRANSACATION TYPE VSAM FILE
- Inputs: inline control data
- Outputs: None found in extracted data
...
- Inputs: AWS.M2.CARDDEMO.TRANTYPE.PS, AWS.M2.CARDDEMO.TRANTYPE.VSAM.KSDS, inline control data
- Outputs: None found in extracted data
- Inline SYSIN: 1 control line(s) are supplied to the step.

## 4. Dataset and Dependency Context

- Input datasets: inline control data, AWS.M2.CARDDEMO.TRANTYPE.PS, AWS.M2.CARDDEMO.TRANTYPE.VSAM.KSDS
...
- Peer/upstream references to the flat-file inputs: DEFGDGD/STEP20
